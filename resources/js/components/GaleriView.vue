    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div v-if="filteredItems.length > 0">
        ...
        <div v-if="isGridView" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <!-- Thumbnail: Foto pakai file_url, Video pakai thumbnail YouTube jika ada -->
            <img
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div class="lg:col-span-5 flex justify-center">
            ...
            >
              <img
                :src="highlightVideoThumb || '/images/hero-bg.png'"
          ...
          <div class="relative bg-black flex items-center justify-center" :class="selectedItem.type === 'video' ? 'aspect-video' : 'max-h-[60vh]'">
            ...
            <template v-if="selectedItem.type === 'video'">
              <iframe
                ...
                allowfullscreen
              ></iframe>
              <!-- Fallback: video upload lokal (file_url) -->
              <video
                v-else-if="selectedItem.image && selectedItem.image.endsWith('.mp4')"
              ...
              <div v-else class="w-full aspect-video bg-gray-900 flex items-center justify-center text-white text-xs text-center p-4">
                <div>
                  ...
                  <p>Video tidak tersedia. Silakan tambahkan URL YouTube.</p>
                </div>
              </div>
            </template>
            ...
            <button
              ...
              class="absolute top-4 right-4 p-2 bg-black/60 hover:bg-black/80 text-white rounded-full transition-colors cursor-pointer z-10"
            >
              <XIcon class="w-5 h-5" />
            </button>
          </div>
        ...
        <div class="bg-white rounded-3xl max-w-lg w-full p-6 md:p-8 shadow-2xl border border-gray-100 space-y-5">
          ...
          <form @submit.prevent="submitUpload" class="space-y-4">
            ...
            <div class="flex justify-end gap-3 pt-2">
              <button type="button" @click="showUploadModal = false" class="px-4 py-2 text-xs font-bold text-gray-500 hover:text-gray-700">Batal</button>
              <button type="submit" class="px-6 py-2.5 bg-[#0D6847] hover:bg-[#0A5238] text-white text-xs font-bold rounded-xl shadow-md cursor-pointer">Kirimkan</button>
            </div>
          </form>
        </div>
      </div>
...
// Kategori dari master_kategori (tipe = galeri)
...
// Highlight video dari store (jika ada is_highlight)
const highlightVideo = computed(() =>
  store.gallery.find(g => g.type === 'video' && g.youtube_url)
  || store.gallery.find(g => g.type === 'video')
);

const highlightVideoThumb = computed(() => {
  if (!highlightVideo.value) return '';
  return getYouTubeThumb(highlightVideo.value.youtube_url) || highlightVideo.value.image;
});

const highlightVideoTitle = computed(() => {
  return highlightVideo.value?.title || 'Profil Keindahan & Budaya Padukuhan';
});
...
const openHighlightVideo = () => {
  if (highlightVideo.value) {
    selectedItem.value = highlightVideo.value;
  } else {
    // Fallback jika tidak ada video highlight
    showToast('Belum ada video profil yang diunggah.');
  }
};

const openUploadModal = () => {
  uploadForm.value.category = categories.value[0] || '';
...
const submitUpload = () => {
  ...
  uploadForm.value = { name: '', title: '', category: '' };
};
