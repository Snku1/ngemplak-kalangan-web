...
class DemographicController extends BaseController
{
    ...
    {
        $data = Demographic::orderBy('kategori')
            ->orderBy('urutan')
            ->orderBy('label')
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($input, [
            'kategori'    => 'required|in:usia,jenis_kelamin,pendidikan,pekerjaan',
    ...
    public function show($id)
    {
        ...
        if (is_null($data)) return $this->sendError('Demographic not found.');
        return $this->sendResponse($data, 'Demographic retrieved successfully.');
    }
